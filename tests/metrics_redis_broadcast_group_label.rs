...
//! Integration test: a broadcast subscription's `consumer_group` label.
//!
...
/// Warm-up probe. `Recorder` acks it and the assertions ignore it.
const WARMUP_KEY: u64 = u64::MAX;
...
shove::define_topic!(
    CacheInvalidations,
    Invalidate,
    TopologyBuilder::new("redis-bcast-group-label")
        .broadcast()
        .build()
);
...
async fn a_broadcast_subscription_labels_every_series_with_its_configured_group() {
    ...
    let broker = {
        ...
        for _ in 0..60 {
            let config = RedisConfig::new(RedisMode::Standalone { url: url.clone() });
        ...
        connected.expect("Redis never became reachable")
    };
    ...
    subscriber
        .subscribe::<CacheInvalidations, _>(
            handler.clone(),
            ConsumerOptions::<Redis>::new().with_consumer_group(GROUP),
        )
        .expect("subscribe");

    // Redis keeps no registry of `XREAD` readers, so a real publish is the only
    // honest readiness signal.
    let mut live = false;
    for _ in 0..200 {
        if handler.saw(WARMUP_KEY) {
            live = true;
            break;
        }
        publisher
    ...
    assert!(live, "the subscription never became live");
