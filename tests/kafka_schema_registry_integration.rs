...
#[derive(Clone)]
struct MockState {
    /// The subject the registry reports the resolved id belongs to.
    subject: Arc<str>,
}
...
async fn schema(State(_s): State<MockState>, Path(_id): Path<u32>) -> Json<serde_json::Value> {
    // Decode-only phase: the raw schema text is not validated against the
    // payload, only `schemaType` matters for wire-format selection.
