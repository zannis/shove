//! Stress benchmarks for shove under heavy message loads (NATS JetStream backend).
//!
...
//!     cargo run -q --example nats_stress --features nats -- --tier extreme
...
#[command(
    name = "nats_stress",
    about = "Stress benchmarks for shove (NATS JetStream)"
)]
struct Cli {
    /// Which tier(s) to run.
...
shove::define_topic!(
    StressTopic,
    StressMsg,
    TopologyBuilder::new("nats-stress-bench")
...
async fn run_scenario(
    ...
    let mut registry = NatsConsumerGroupRegistry::new(broker.client.clone());
...
async fn main() {
    ...
    eprintln!(
        "shove NATS JetStream stress benchmarks{}",
