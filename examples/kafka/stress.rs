//! Stress benchmarks for shove under heavy message loads (Kafka backend).
//!
...
//!     cargo run -q --example kafka_stress --features kafka -- --tier extreme
...
#[derive(Parser)]
#[command(name = "kafka_stress", about = "Stress benchmarks for shove (Kafka)")]
struct Cli {
    /// Which tier(s) to run.
...
shove::define_topic!(
    StressTopic,
    StressMsg,
    TopologyBuilder::new("kafka-stress-bench")
...
impl Broker {
    async fn start() -> Self {
        ...
        declarer.declare(StressTopic::topology()).await.unwrap();
...
async fn run_scenario(
    ...
    let mut registry = KafkaConsumerGroupRegistry::new(broker.client.clone());
