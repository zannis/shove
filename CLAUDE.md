...
- `HasBatchConsumption` gates `Broker::batch_consumer` /
  `BatchConsumer<B>::run`. Kafka, InMemory and SQS implement it today (SQS
  with a hard 10-message cap — `max_batch_size > 10` is rejected at consumer
  startup); every remaining backend is pending, not excluded — each gets the
  capability the moment its own `BatchConsumerImpl` lands. The primitive
  exists for **handler amortisation** (one flush per N messages instead of
  one call per message), nothing else.
