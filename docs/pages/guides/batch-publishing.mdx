...
| RabbitMQ | **prefix on a NACK, tail otherwise** | Every record is submitted, then confirms are awaited in order. A NACK stops the call: `failed()` is that one index, `unattempted()` is everything after it. A `basic_publish` or confirm *error* names nothing as rejected — see the rule below. |
| Redis | **prefix** | Sequential; stops at the first error. |
| InMemory | **prefix** | Sequential; stops at the first error. |

`failed()` means the broker gave a verdict — nothing else earns it. A transport failure says nothing about whether the record was stored, so RabbitMQ (a failed `basic_publish` or confirm) and NATS (an ack that timed out or died with the connection) report those indices as `unattempted()` instead. The re-publish set is identical either way; only the diagnosis differs.
