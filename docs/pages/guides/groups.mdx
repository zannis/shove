...
> **Note:** `.with_concurrent_processing(true)` applies to `register` only. Every backend whose config carries the flag rejects it in `register_fifo`, returning a `ShoveError::Topology` at registration that names what to drop — it is never silently discarded. Concurrency for a sequenced topic comes from the shard fan-out instead (and on RabbitMQ additionally from `prefetch_count`, which bounds how many distinct sequence keys run at once). InMemory has no such flag.
