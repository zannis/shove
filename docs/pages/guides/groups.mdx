...
**Redis (`redis::RedisConsumerGroupConfig`)** — adds `.with_handler_timeout(Duration)` mirroring every other backend. Redis Streams uses a fixed consumer count per registration (set via `RedisConsumerGroupConfig::new(consumer_count: u16)`); there is no concurrent-processing toggle.
...
1. **Per-group explicit** — the config called `.with_handler_timeout(d)` (or `.without_handler_timeout()` on backends that expose it). This always wins.
