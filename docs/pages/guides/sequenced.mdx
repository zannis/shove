...
For consumer groups, `register_fifo` takes only the handler factory — there is no `ConsumerGroupConfig` parameter because FIFO concurrency is determined by the topology's `routing_shards`, not by user-supplied min/max. Autoscaling is not applied to FIFO registrations (more replicas would break per-key ordering):

```rust,no_run
let mut group = broker.consumer_group().with_context(ctx);
group.register_fifo::<SequencedTopic, _>(|| MyHandler).await?;
