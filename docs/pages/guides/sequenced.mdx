...
Most pub/sub messages don't need ordering. When they do — e.g. ledger entries for the same account, state transitions for the same order, audit events for the same entity — `shove` lets you mark a topic as **sequenced** so messages with the same key are delivered in strict publish order. Messages sharing the same sequence key are consumed one at a time: the next delivery only happens after the previous one has been successfully acked.

## When to use sequenced topics

Use a sequenced topic when messages for the same logical entity have **causal dependencies** — when processing message N assumes message N−1 has already been applied:
...
**`SequenceFailure::Skip`** — dead-letters the failed message and continues processing subsequent messages in the sequence as normal. The failed message is skipped; messages 4 and 5 are delivered to the handler as if message 3 never existed.

Use `Skip` when messages are **independently valid** but happen to require ordered delivery. Audit log entries are a good example: if entry 3 is malformed and cannot be processed, entries 4 and 5 are still valid records that should be stored. The audit trail has a gap, but it continues.
