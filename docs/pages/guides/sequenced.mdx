...
`FailAll` is honoured on all six backends, with two limits worth knowing:

- **The poison record is process-local.** It survives a broker reconnect, but it is not shared between consumers. A second consumer of the same topic keeps its own set and will still invoke the handler for a key the first one poisoned; on Kafka, a partition moved by a rebalance likewise arrives at its new owner unpoisoned. Ordering is preserved in both cases — only the poison record is lost.
- **NATS carries the key in a `Shove-Sequence-Key` header** added in the release that brought `FailAll` to NATS. Messages published by an older version carry no key and are never poisoned, so they degrade to `Skip` rather than taking out the shard. Messages in flight across an upgrade are safe; drain them, or republish, before relying on `FailAll` on NATS.
...
| Redis/Valkey Streams | Per-shard stream `{queue}-seq-{shard}` | One sequential reader per shard stream |
...
Returns the same `SupervisorOutcome` as `ConsumerGroup::run_until_timeout`, so process-level exit-code rollups across coordinated groups and sequenced topics use one type. See [Shutdown semantics → Sequenced topics](/guides/shutdown#sequenced-topics-run_fifo_until_timeout) for the full contract.
