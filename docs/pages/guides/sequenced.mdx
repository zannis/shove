...
**The generic `broker.consumer_group()` harness does not currently route to the FIFO path** — spawned workers call the unsequenced `run`. To prevent per-key ordering being silently dropped, both `ConsumerGroup::register` and `ConsumerSupervisor::register` reject any topic whose topology has `.sequenced(...)` configured: registration returns `ShoveError::Topology(...)` at runtime. To preserve ordering on a coordinated backend, drive the consumer through `run_fifo` directly until FIFO registration is surfaced on the harness.
