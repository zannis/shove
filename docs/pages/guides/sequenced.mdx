...
**The generic `broker.consumer_group()` harness does not currently route to the FIFO path.** Spawned workers call the unsequenced `run` regardless of whether the topic implements `SequencedTopic`, so per-key ordering is silently dropped. To preserve ordering on a coordinated backend, drive the consumer through `run_fifo` directly. `ConsumerSupervisor::register` rejects sequenced topics at runtime to surface the same trap loudly; `consumer_group()` does not yet, so prefer `run_fifo` for any topic declared with `.sequenced(...)`.
