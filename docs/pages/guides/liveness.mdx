...
| NATS      | subscribe + publish + await echo on a unique inbox | none                     |
...
`Broker::ping` uses `DEFAULT_PING_TIMEOUT` (5 s). Match it to your platform's probe configuration:

```rust,no_run
use std::time::Duration;
use shove::{Broker, Kafka};

async fn readiness(broker: &Broker<Kafka>) -> http::StatusCode {
    let deadline = Duration::from_secs(2);
    match broker.ping_with_timeout(deadline).await {
        Ok(()) => http::StatusCode::OK,
        Err(_) => http::StatusCode::SERVICE_UNAVAILABLE,
    }
}
...
Align `deadline` with your k8s `timeoutSeconds`. If the underlying RPC exceeds the deadline, `ping` returns `Err(ShoveError::Connection(...))` whose message names the timeout.
...
- **No per-topic check.** `ping` answers "can I talk to the cluster", and that alone — declaring topic X is a topology concern, handled elsewhere.
