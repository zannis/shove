...
document, the in-process batch flow drains 2.7-10.5x faster than the plain
parallel consumer at 64 B and 1.9-7.0x faster at 1 KiB, and 2.0-3.8x slower at
64 KiB — so what turns the comparison around is payload size. Every bound is
rounded outward, so no drain row falls outside a range stated here. Those 64 KiB
rows are read from the document rather than off the charts: every in-process
64 KiB drain in the published run finished under the harness's reliability
floor, so the charts withhold all of them. Batch size is not the lever either — an
