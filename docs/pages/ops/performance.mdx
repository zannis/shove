...
handler call itself. It does not pay for large payloads: on the in-process
backend at 64 KiB, a 500-message batch (a 32 MB flush) drained at 55k msg/s,
a 50-message batch at 80k, a 10-message batch at 79k, and the plain parallel
consumer at about 356k. Below about 1 KiB the batch path leads; at 64 KiB it
trails by 4x whatever the batch size, so the extra cost is per message, not
per flush. Batch small messages; consume large ones one at a time.
