...
**NATS**, **RabbitMQ** and **Redis** backends. SQS is the one backend absent: it
runs only against LocalStack (non-representative by design), needs a LocalStack
Pro auth token, and is not in this document. The harness refuses to merge runs
measured on different hosts into one document, so completing the set means
measuring SQS on the same host as the published runs.
