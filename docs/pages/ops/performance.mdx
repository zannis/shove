...
rate it sustained — is a cell where a batch fills in 5 ms to 100 ms and so never
waits out the age bound, which is where the effect is at its smallest. A batch
series drawn under this chart's own policy would understate the cost rather than
publish it. The measured cost is stated as a number under
...
— and that wait is the price of the amortisation. What it costs is decided by
which of the two bounds fires, so the two regimes are worth reading separately
rather than pooled.

The comparison throughout is a **paired offered-load rung**: one `consume_batch`
row against the plain parallel consumer at the same backend, payload, consumer
count and offered rate, both arms sustaining the rate, every batch row at the
defaults (`max_batch_size` 500, `max_batch_age` 250 ms). The document has 120 of
them, across five backends.

**When the batch cannot fill, the age bound is the whole cost.** A consumer has
to see 2,000 msg/s to collect 500 messages inside 250 ms. In the **30** rungs
below that — 625 to 1,250 msg/s per consumer, six on every backend — batch
dispatch p99 is **250-262 ms in all 30**, against **0.14-144 ms** for the
comparator, and batch is the slower arm in **30 of 30**. By backend the ratio is
**145x-1744x** in-process, **14.7x-22.2x** on Kafka, **1.7x-97.7x** on NATS,
**4x-98.9x** on RabbitMQ and **66x-97x** on Redis, six rungs each. That spread
is the comparator's, not the batch side's: batch p99 sits within 12 ms of the
age bound in every one of the 30, while the parallel consumer ranges from
0.14 ms in-process at 64 B to 143.74 ms on NATS at 64 KiB. Read the absolute
number, not the multiple — if a consumer is not fed 2,000 msg/s, batching costs
it about a quarter of a second at p99, whatever the backend.

**When the batch can fill, the wait shrinks with the fill.** Across the other
**90** rungs batch p99 runs **6.1-251 ms** and batch is the slower arm in
**85 of 90**; at 100,000 msg/s, where a consumer fills 500 messages in 5 ms to
40 ms, it is down to **6.1-104 ms** over sixteen rungs. The five rungs where
batch is *not* slower are all in-process, and all of them are cases where the
parallel consumer was itself the slower arm — its own p99 reaches 339.2 ms at
64 KiB, against batch's 234.7 ms at the same cell. Pooled over all 120 rungs
batch loses 115, but that pooled figure is the weakest way to read the trade: it
mixes a regime where the cost is fixed at the age bound with one where it tracks
the arrival rate.

So the lever is `max_batch_age` — lower it to shorten the wait, and pay for it
in smaller average batches and less amortisation. Every bound here is rounded
outward, so no rung falls outside a range stated; note that the largest measured
p99 is 261.85 ms, *above* the 250 ms age bound, so read 250 ms as the wait a
message can accrue before a flush is triggered and not as a cap on the
percentile. SQS contributes no rung: all twelve of its batch offered-load rows
are `sustained: false`, so their percentiles are backlog residency and not
dispatch, and the harness withholds them.
