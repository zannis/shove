...
The document these charts render from measures the **in-process** and **Kafka**
backends. NATS, RabbitMQ and Redis are supported by the harness but are not in
this document yet; SQS additionally only runs against LocalStack
(non-representative by design), which needs a LocalStack Pro auth token. The
harness refuses to merge runs measured on different hosts into one document, so
completing the set means re-measuring every backend back to back on a single
host rather than appending to what is published here — that rerun is pending.
