...
an explicit "not supported" marker, never as a zero. And SQS is not measured
in this document at all: it only runs against LocalStack (non-representative
by design), which requires a LocalStack Pro auth token that was unavailable
where these results were generated — the charts carry an explicit "not
measured" note for it instead of a plotted series.
