...
**Stream-side vs consumer-side limits.** Storage size and per-message size are set on the JetStream stream itself (configured outside shove). `max_ack_pending`, however, is set by shove on the durable pull consumer at consumer-creation time. For a single consumer it is derived from `prefetch_count`. For a group it is the aggregate the group can actually hold in flight: `max_consumers × prefetch_count` with concurrent processing on, and `max_consumers` in the default sequential mode, where each member holds one message at a time. To override either, use `ConsumerOptions::<Nats>::with_max_ack_pending(...)` on a single consumer, or `NatsConsumerGroupConfig::with_max_ack_pending(...)` to replace the derived aggregate for a group. Both take a positive budget or `-1` for unbounded, and reject `0` — JetStream reads a zero as "unset" and silently substitutes its own default of 1000.

The **batch consumer** is the exception, and it has no equivalent knob: when nothing has pre-declared the durable, its bootstrap creates one with `max_ack_pending` **unbounded**. JetStream serves every concurrent pull request on a durable from that one budget, so a finite budget is split across the attached batch workers; no request can then fill to `max_batch_size`, every partial pull sits open until the server-side expiry, and throughput on a full stream collapses to `max_batch_size / max_batch_age`. In-flight exposure stays bounded by the pull requests themselves — at most one batch per worker, settled at flush — not by the budget. If you want a hard aggregate cap anyway, pre-declare the durable with one, but size it for the number of batch workers you will attach rather than for a single batch. shove warns at startup when it attaches to a durable whose budget covers only one batch.

**Bounding reconnects in a group.** `NatsConsumerGroupConfig::with_max_reconnect_attempts(n)` bounds each group consumer task's *consecutive* reconnect attempts, the same way `ConsumerOptions::<Nats>::with_max_reconnect_attempts` does for a single consumer; the count resets after a sustained healthy period. Leaving it unbounded (the default) lets reconnect backoff grow past `ack_wait`, at which point in-flight messages expire during the backoff itself.
...
**SASL/TLS.** Enable the `kafka-ssl` feature for TLS transport.
`KafkaSasl` configuration supports PLAIN, SCRAM-SHA-256 and SCRAM-SHA-512.
librdkafka implements PLAIN itself in every build.
SCRAM and OAUTHBEARER come from its OpenSSL build, which `kafka-ssl` selects.
...
For Amazon MSK with IAM authentication, enable the `kafka-msk-iam` feature instead, and see the [Kafka backend page](/backends/kafka) for setup details.

**`librdkafka` system dependency.** `kafka-ssl` links OpenSSL and nothing else.
Only `kafka-gssapi` needs the SASL development headers, and on Debian/Ubuntu CI runners you install them before building:

```sh
apt-get install -y libsasl2-dev
```

On macOS, Homebrew's `cyrus-sasl` satisfies it.
`librdkafka` itself is not a runtime dependency: `rdkafka-sys` compiles the librdkafka source it bundles and links it statically, so the binary carries it.
Only OpenSSL, and Cyrus SASL under `kafka-gssapi`, are linked from the system.
The `rdkafka` crate's `dynamic-linking` feature is the opt-in exception, and shove does not enable it.

**Consumer offsets are committed asynchronously, at most once per commit interval.** shove tracks handler completions and commits them asynchronously every 500 ms by default.
`with_commit_interval` on `KafkaConsumerGroupConfig` or `ConsumerOptions::<Kafka>` changes the cadence, trading coordinator requests for a wider replay window.
On clean shutdown, shove first waits for the in-flight handlers to finish.
That wait is bounded by the handler timeout and by any in-place delays on an external topic.
It then commits the final position synchronously from a dedicated thread and waits at most 20 s for that commit.
That 20 s bound covers the concurrent consumer's final commit only.
A synchronous commit already in flight when shutdown fires, the one that retires a discarded record, blocks for as long as librdkafka retries it.
A FIFO consumer commits a discarded message synchronously and a batch consumer commits every flush synchronously, and those commits block the same way.
On a crash or SIGKILL, expect at-least-once redelivery from the last committed offset when the consumer restarts.
Design handlers to be idempotent or use the dedup layer to handle this.
