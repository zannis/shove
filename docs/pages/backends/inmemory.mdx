...
- [Liveness Probes](/guides/liveness) — wire `Broker::ping` into a k8s health endpoint. The in-process backend's `ping` checks the shutdown token and performs no I/O.
