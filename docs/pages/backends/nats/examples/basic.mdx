...
This example demonstrates the full publish/consume cycle using NATS JetStream: declare a topic backed by a JetStream stream, publish three `OrderCreated` events, and consume them with a coordinated consumer group. It is the canonical starting point for NATS JetStream integration, showing how shove's generic `Broker\<Nats\>` API maps to JetStream streams and durable consumers.
