...
This example measures the throughput difference between sequential and concurrent message processing for a slow handler (100 ms per message). By running both modes against the same `SlowTasks` topic and printing a speedup ratio, it makes the cost of I/O-bound sequential processing tangible and shows how `ConsumerOptions::with_concurrent_processing` eliminates that cost while preserving in-order acknowledgement.
