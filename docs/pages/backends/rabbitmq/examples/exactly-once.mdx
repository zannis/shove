...
This example demonstrates how `ConsumerOptions::<RabbitMq>::with_exactly_once()` wraps every routing decision in an AMQP transaction, closing the window between the hold-queue publish and the ack that can produce duplicate deliveries under at-least-once semantics. Three topics exercise the three routing outcomes in transactional mode: ack, retry-then-ack, and reject-to-DLQ.
