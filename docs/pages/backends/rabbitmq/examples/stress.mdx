...
When `--concurrent` is passed on the command line, `concurrent=true` enables the overlap mode from the [concurrent example](/backends/rabbitmq/examples/concurrent); this is especially effective for `slow` and `heavy` handler profiles where I/O dominates.
