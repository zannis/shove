...
Use this as your starting point for RabbitMQ integration: it covers every non-sequenced topology shape (minimal queue, DLQ, escalating retry backoff, deferred delivery), all four `Outcome` variants, and all three publish variants against a real broker. The canonical reference for how [topic definitions](/concepts/topics) and [handler outcomes](/concepts/outcomes) map to RabbitMQ.
