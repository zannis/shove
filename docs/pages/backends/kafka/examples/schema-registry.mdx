...
Any auth other than `None` against an `http://` base URL makes `build()` panic — the bearer token or basic-auth password would otherwise be sent in cleartext on every schema fetch, readable by anything on the path. An unauthenticated `http://` registry keeps working unchanged.
