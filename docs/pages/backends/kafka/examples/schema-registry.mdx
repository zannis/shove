...
Reach for `non_secret_header` rather than `allow_plaintext_credentials()` when a benign header is the only thing tripping the guard — that opt-in is not scoped to a single header, so using it to quiet an `Accept` header would also permit a real bearer token in cleartext.
