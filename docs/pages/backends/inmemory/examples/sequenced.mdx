...
This example shows per-key FIFO delivery using the in-memory backend's `run_fifo` path. Messages for three accounts (`alice`, `bob`, `carol`) are interleaved in publication order, yet each account's entries are consumed strictly in sequence — demonstrating [sequenced delivery](/guides/sequenced) and the `SequenceFailure::FailAll` error policy without any external infrastructure.
