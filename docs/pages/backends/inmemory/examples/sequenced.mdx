...
- Change `SequenceFailure::FailAll` to `SequenceFailure::Skip` — observe that only the individual failing message is dead-lettered; subsequent messages for the same account are still processed.
