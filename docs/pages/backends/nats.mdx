...
- **JetStream consumer tuning is fixed.** Streams are declared with `RetentionPolicy::WorkQueue`, `StorageType::File`, and `DiscardPolicy::New`; durable consumers use `AckPolicy::Explicit` with `max_ack_pending = prefetch × max_consumers` and a 120 s deduplication window. These are not yet user-configurable — open an issue if you need them surfaced as knobs.
