...
username/password and token credentials require a `tls://` or `nats+tls://`
URL so reusable secrets are never sent in cleartext. A local development broker
that deliberately uses plaintext must opt in with
`config.allow_plaintext_credentials = true`. The `Debug` impl and connection errors
redact every credential and the user-info portion of the URL.
