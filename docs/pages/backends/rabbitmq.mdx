...
A classic queue is a single Erlang process, so its throughput is bounded by
one core on the broker whatever the consumer count. On RabbitMQ 3.8.22 in the
benchmark VM that ceiling is about 35k msg/s at 64 B in either direction:
eight publishers fill a queue at 34k msg/s, the batch consumer drains it at
32k to 35k at one, two, four and eight consumers alike, and a connection per
consumer changes nothing (measured 2026-09-10: 35.7k on one shared connection,
31.1k on four). Adding consumers to one queue spreads handler work; it does
not add broker throughput. To scale past the ceiling, spread the topic over
several queues: a sequenced topic with `routing_shards(N)` does exactly that
on the broker side, at the cost of per-key ordering constraints.
...
- [Broadcast](/concepts/broadcast) — per-instance fan-out via a `{queue}-fanout` exchange, which changes the publish route too.
