...
This example runs the same `SkipLedger` vs `StrictLedger` comparison found in the [RabbitMQ sequenced example](/backends/rabbitmq/examples/sequenced), but against an SQS backend via LocalStack. Messages for two accounts are published in interleaved order; `SequenceFailure::Skip` lets the sequence continue after a single poisoned entry while `SequenceFailure::FailAll` quarantines all subsequent entries for the same key. The example asserts exact totals to prove the policies behave correctly.
