...
All three panic on an empty or a negative requirement, or on one longer than 1024 indexes: the parser never yields one, so it could never match, and every protobuf frame would be dead-lettered with nothing at startup pointing at the cause.
