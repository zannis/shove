...
A publish never creates a topic.
...
The producer never creates the topic it publishes to; see [Declare topology](#declare-topology).
...
- **A publish to an undeclared topic fails.** The producer pins `allow.auto.create.topics=false`, so a broker with `auto.create.topics.enable=true` no longer creates a topic with broker defaults on the first publish. Call `topology().declare::<T>()` before the first publish, or provision the topic outside shove. The retry and DLQ publishes of a consumer are unaffected, because `declare` creates those topics.
