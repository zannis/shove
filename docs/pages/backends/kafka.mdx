...
OAUTHBEARER is compiled in, but shove exposes it only as `KafkaSasl::MskIam` under `kafka-msk-iam` (below); there is no generic token provider.
...
- **SASL and TLS** require the `kafka-ssl` feature.
  ...
  Shove exposes OAUTHBEARER only as `KafkaSasl::MskIam` under `kafka-msk-iam`; there is no generic token provider.
