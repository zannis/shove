...
Publishing is outside it: a `Publisher` on this topology writes to the topic, and the producer keeps librdkafka's default of `allow.auto.create.topics=true`, so a publish to a topic that does not exist can create it with broker defaults on a broker with `auto.create.topics.enable=true`.
Provision the topic before any publisher starts, or pin the flag on the producer if that behaviour ever changes library-wide.
...
On a topic bound with `external()` there is no republish.
