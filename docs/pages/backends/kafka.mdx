...
- **`with_auto_offset_reset(KafkaAutoOffsetReset)`** — `Earliest` (default, replay history), `Latest` (tail-only), or `None` (refuse silent replay/skip on a fresh group).
  ...
  The DLQ drain (`run_dlq_with_options`) refuses the setting: a drain always starts a fresh group at the earliest retained offset, so it never skips a dead letter.
...
If that thread cannot be spawned either, the handle is leaked and logged, and the broker drops the member after the session timeout, as after a crash.
...
All three panic on an empty or a negative requirement: the parser never yields one, so it could never match, and every protobuf frame would be dead-lettered with nothing at startup pointing at the cause.
