...
By default a publish never creates a topic.
The producer runs with `allow.auto.create.topics=false`, so a publish to a topic nobody declared fails with a `Connection` error after the produce timeout, and the topic stays absent.
Declare the topology before the first publish, or provision the topic outside shove.
To let a producer create a missing topic on a broker that runs `auto.create.topics.enable=true`, set `KafkaConfig::with_producer_auto_create_topics(true)`; the topic then gets broker defaults, outside any topology config.
...
By default the producer never creates the topic it publishes to; see [Declare topology](#declare-topology).
...
- **A publish to an undeclared topic fails by default.** The producer sets `allow.auto.create.topics=false`, so a broker with `auto.create.topics.enable=true` no longer creates a topic with broker defaults on the first publish. Call `topology().declare::<T>()` before the first publish, provision the topic outside shove, or opt back in with `with_producer_auto_create_topics(true)` on `KafkaConfig`. The retry and DLQ publishes of a consumer are unaffected, because `declare` creates those topics.
