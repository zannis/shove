...
The `kafka-schema-registry` feature adds decode-only [Confluent Schema Registry](https://docs.confluent.io/platform/current/schema-registry/index.html) support for Kafka consumers. When enabled, each incoming message is unwrapped from the Confluent wire frame (magic byte `0x00` + 4-byte big-endian schema id; for Protobuf, also the message-index array), the schema id is resolved against the registry (cached in memory, with single-flight deduplication of concurrent cold misses and a configurable negative-TTL for permanent failures), and the inner payload is decoded via the topic's existing `Codec` (`JsonCodec` or `ProtobufCodec`).
