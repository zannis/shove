...
`register_fifo` wires consumer tasks for sequenced delivery. `RedisConsumerGroupConfig::default()` spawns one consumer task; use `RedisConsumerGroupConfig::new(n)` to run more. See [Sequenced Topics](/guides/sequenced) for the full ordering model.
