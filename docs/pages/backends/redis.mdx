...
- **Trimming runs on a sweep, not on ack.** `XACK` clears the pending entry; the entry itself leaves the stream at the next sweep, which runs every handler timeout floored at 30 s. At a high publish rate that is up to 30 s of consumed traffic resident in memory. `RedisConfig::with_trim_interval(Duration)` sets the sweep cadence directly (the reclaim deadline stays on the handler timeout); each sweep is `XINFO GROUPS`, one `XPENDING` per group and one `XTRIM`, so a sub-second interval trades round trips for memory.
...
- [Autoscaler](/backends/redis/examples/autoscaler) — consumer-lag-driven scale up/down within a `min..=max` range
...
- [Broadcast](/concepts/broadcast) — per-instance fan-out via plain `XREAD`, with no group and nothing to reap. Bound the stream first.
