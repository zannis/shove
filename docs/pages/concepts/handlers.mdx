...
Most handlers ignore all of these fields. They become useful when you need to trace a message (`headers["x-trace-id"]`), implement idempotency checks (`headers["x-message-id"]` on RabbitMQ), or adjust behavior on redelivery. The `retry_count` in particular is more reliable than `redelivered` for detecting retries, because `shove` owns its meaning, whereas `redelivered` reflects the broker's view of the transport. Under `RetryStrategy::Republish` `shove` increments the count itself and carries it in a header. Under `RetryStrategy::InPlace` no copy is published: Kafka keeps the count in memory for the redelivery, and an external NATS stream reads it from the JetStream delivery information, which counts every redelivery.

### `retry_count` vs `delivery_count`

The two counters answer different questions, and neither is a total-attempts-ever number.

`retry_count` is `shove`'s retry budget, the field `max_retries` and DLQ routing are measured against. How it advances depends on the retry strategy. Under `RetryStrategy::Republish`, the default on a shove-owned topology on every backend, it advances only when `Outcome::Retry` republishes the message, into a hold queue on the backends that have one (NATS, Redis, RabbitMQ, SQS and the in-process broker) or back into the topic on Kafka, and it survives that hop because `shove` carries it in a header. Under `RetryStrategy::InPlace` on Kafka, implied by an external topology, it advances on each `Retry` the handler's task waits out, and is kept in memory for the redelivery. Under `RetryStrategy::InPlace` on an external NATS stream it is the number of redeliveries JetStream reports, so a `Defer` nak and an `ack_wait` expiry advance it too.
