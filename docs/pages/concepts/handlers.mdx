...
Because the count belongs to a *broker-level* message, anything that creates a new one restarts it at 1. `Retry` publishes an incremented copy on every backend, so it always resets `delivery_count` — `retry_count` is the field that survives a retry. `Defer` resets it where deferring re-sends the message (SQS) and preserves it where deferring naks in place (NATS, and the in-process broker, which models NATS here).

That last case is what `delivery_count` is for; see [Bounding a `Defer` loop](/guides/retries#defer-vs-retry).

On Kafka, a consumer running with `RetryStrategy::InPlace`, which an external topology implies and a shove-owned one may opt into, publishes no copy on `Retry` or `Defer`.
