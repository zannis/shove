...
- `offset: Option<i64>` - the record's offset inside its partition, on Kafka only, and `None` on every other backend.
- `timestamp_ms: Option<i64>` - the record's broker timestamp in milliseconds since the Unix epoch, `LogAppendTime` or `CreateTime` as the topic is configured.
  Kafka only, and `None` when the record carries none.

The three Kafka coordinates name a position in the log, which is what an audit trail or a replay request needs.
...
On a Kafka topic bound with `kafka_external_topic()` neither `Retry` nor `Defer` publishes a copy.
