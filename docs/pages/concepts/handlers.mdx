...
- `partition: Option<i32>` - the partition the record was read from, on Kafka only, and `None` on every other backend.
- `offset: Option<i64>` - the record's position in its log: the offset inside its partition on Kafka, the stream sequence on NATS, and `None` elsewhere.
- `timestamp_ms: Option<i64>` - when the broker took the message, in milliseconds since the Unix epoch, and `None` where the backend has no such time.

Each field is filled where the backend has the data, and each has its own availability:

| Backend | `partition` | `offset` | `timestamp_ms` |
|---|---|---|---|
| Apache Kafka | `Some(p)` | `Some(o)`, the offset inside the partition | `Some(ms)`, the broker timestamp (`LogAppendTime` or `CreateTime` as the topic is configured); `None` when the record carries none |
| NATS JetStream | `None` | `Some(seq)`, the stream sequence, checked into an `i64` | `Some(ms)`, when the server received the message from its publisher |
| Redis Streams | `None` | `None` | `Some(ms)`, the time component of the entry id: the instance clock when Redis generated the id. Not a publish time in general: a publisher may supply an explicit id, and after a clock rollback Redis reuses the top entry's time |
| RabbitMQ | `None` | `None` | `None` on this version; the optional AMQP `timestamp` property is not read |
| AWS SQS | `None` | `None` | `None` on this version; `SentTimestamp` is not requested |
| In-process | `None` | `None` | `None` |

The coordinates name a position in a log, which is what an audit trail or a replay request needs.
They are not an attempt counter: the same record redelivered after a rebalance carries the same coordinates.
On a dead-letter drain the coordinates are the dead letter's own, the copy's position in the dead-letter topic, while `retry_count` and `delivery_id` describe the original message.

Most handlers ignore all of these fields.
They become useful when you need to trace a message (`headers["x-trace-id"]`), implement idempotency checks (`headers["x-message-id"]` on RabbitMQ), or adjust behavior on redelivery.
The `retry_count` in particular is more reliable than `redelivered` for detecting retries, because `shove` defines how it advances.
`redelivered` only reflects the broker's view of the transport.
Under `RetryStrategy::Republish`, `shove` increments the count itself and carries it in a header on the copy it publishes.
Under `RetryStrategy::InPlace` no copy is published.
Kafka then keeps the count in memory for the redelivery.
An external NATS stream reads it from the JetStream delivery information, which counts every redelivery.

### `retry_count` vs `delivery_count`

The two counters answer different questions, and neither is a total-attempts-ever number.

`retry_count` is `shove`'s retry budget, the field `max_retries` and DLQ routing are measured against.
How it advances depends on the retry strategy and the backend.
Under `RetryStrategy::Republish`, the default on a shove-owned topology, it advances only when `Outcome::Retry` publishes an incremented copy.
Redis, RabbitMQ, SQS and the in-process broker route the copy through a hold queue for the tier's delay.
Kafka has no hold queue and republishes the copy into the consumed topic after the delay.
NATS has no hold stream either.
It holds the original un-acked for the delay, with progress acknowledgements, then publishes the copy to the same subject and acks the original.
In every case the count survives the hop because `shove` carries it in a header.
Under `RetryStrategy::InPlace` on Kafka, implied by an external topology, no copy is published.
The count advances on each `Retry` the handler's task waits out, and is kept in memory for the redelivery.
Under `RetryStrategy::InPlace` on an external NATS stream, the count is the number of redeliveries JetStream reports.
A `Defer` nak and an `ack_wait` expiry are redeliveries there, so they advance it too.
...
| RabbitMQ | `None` | AMQP 0-9-1 carries only the `redelivered` flag |
...
Because the count belongs to a *broker-level* message, anything that creates a new one restarts it at 1. Under `RetryStrategy::Republish`, the default on a shove-owned topology, `Retry` publishes an incremented copy and so resets `delivery_count`; `retry_count` is the field that survives a retry. Under `RetryStrategy::InPlace`, Kafka on an external or opted-in topology and NATS on an external stream, no copy is published and the same record comes back: Kafka keeps `retry_count` in memory, and NATS reads it from this very count, which climbs with every redelivery there. `Defer` resets `delivery_count` where deferring re-sends the message (SQS) and preserves it where deferring naks in place (NATS, and the in-process broker, which models NATS here).

That last case is what `delivery_count` is for; see [Bounding a `Defer` loop](/guides/retries#defer-vs-retry).

A consumer running with `RetryStrategy::InPlace` publishes no copy on `Retry` or `Defer`: on Kafka, where an external topology implies it and a shove-owned one may opt into it, the handler's task waits and hands the record back; on an external NATS stream the broker redelivers after a delayed `Nak`.
The same record comes back in place.
On Kafka its `retry_count` is kept in memory for the redelivery.
On an external NATS stream it is read from the JetStream delivery information, which counts every redelivery.
