...
On a Kafka topic bound with `kafka_external_topic()` neither `Retry` nor `Defer` republishes anything.
The handler's task waits the same delay in place and is handed the same record again, with the retry count kept in memory.
