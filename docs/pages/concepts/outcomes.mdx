...
Kafka honours both on this version: an external topology implies `InPlace` and refuses `Republish`, and a shove-owned topology runs with `Republish` by default and may opt into `InPlace` with `with_retry_strategy`.
Under `InPlace` nothing is republished and the retry count is kept in memory.
Every other backend carries a retry through its hold queues on this version, and gains the setter as it declares its support.
See [Bind to an infra-owned topic](/backends/kafka#bind-to-an-infra-owned-topic).
