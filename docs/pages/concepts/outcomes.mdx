...
How a consumer carries out `Retry` and `Defer` is its `RetryStrategy`: `Republish`, an incremented copy after the tier's delay, or `InPlace`, the same record delivered again after the tier's delay with nothing published.
`InPlace` says what is not done, a replacement publish; how the record comes back is the broker's own primitive, in the table below: Kafka waits inside the handler's task and hands the record back, NATS lets JetStream redeliver after a delayed `Nak`.
One invariant binds every backend: a consumer never writes into an external topology when it settles an outcome, so an external topology implies `InPlace` everywhere, and each broker has a native primitive for it.

| Backend | In-place primitive | This version |
|---|---|---|
| Apache Kafka | pauses the assignment and waits in the handler's task, the count kept in memory | both strategies; `with_retry_strategy` on `ConsumerOptions::<Kafka>` and `KafkaConsumerGroupConfig` |
...
On Kafka a shove-owned topology runs with `Republish` by default and may opt into `InPlace` with `with_retry_strategy`; an external topology refuses `Republish`.
Under `InPlace` nothing is republished: Kafka keeps the count in memory, and NATS reads it from the delivery info, so on an external stream a `Defer` and an `ack_wait` expiry consume the retry budget too.
See [Bind to an infra-owned topic](/backends/kafka#bind-to-an-infra-owned-topic) and [Bind to an externally-provisioned stream](/backends/nats#bind-to-an-externally-provisioned-stream).
...
**Handler timeout.** Configured via `ConsumerOptions::with_handler_timeout`. If the handler future does not complete within the timeout, the consumer aborts it and by default treats the result as `Outcome::Retry`, so the message is retried according to normal retry math. Which outcome a timeout resolves to is configurable via `with_handler_timeout_outcome` — see [Choosing what a timeout means](/concepts/handlers#choosing-what-a-timeout-means). Note that a timeout resolved to `Defer` is subject to the sequenced-consumer restriction above.
