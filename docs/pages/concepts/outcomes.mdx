...
How a consumer carries out `Retry` and `Defer` is its `RetryStrategy`: `Republish`, an incremented copy after the tier's delay, or `InPlace`, the same record handed back after waiting the delay inside the handler's task.
