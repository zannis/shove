...
- `.broadcast()` — every *instance* gets its own ephemeral subscription and receives every message, instead of the instances sharing one queue. Combining it with `.for_consumer_group()`, `.dlq()`, `.dlq_named()`, `.hold_queue()` or `.sequenced()` panics at `.build()`, and it implies `.allow_message_loss()`. See [Broadcast](/concepts/broadcast).
...
|---------|----------------------------------|
| Kafka | Yes — the `group.id` becomes `{queue}-{group}-consumer`, so the readers get independent partition assignments. An explicit `with_group_id` still wins. |
| Redis Streams | Yes — give the second reader a client configured with a different XGROUP name; the topology group namespaces its chain to match. |
| NATS, RabbitMQ, SQS | Partly — the retry chain is namespaced, but the readers still share one durable consumer / queue and compete for messages. Real fan-out needs a second stream consumer or a fan-out exchange, which shove does not derive for you. |
...
`TopologyBuilder::external()` binds to infrastructure that infra owns instead of creating it: one flag on every backend, with each declarer deciding what verification means.
On Kafka and NATS, `declare` verifies that the topic or stream exists and fails fast when it does not, while shove still creates its own DLQ.
RabbitMQ, SQS and Redis refuse an external topology at `declare` on this version, until their declarers gain a verification step, because creating the resource is the write the flag promises never to make; the in-process broker treats the flag as a no-op.
Their consumers refuse it too, at every entry point and before any request, because a consumer can run without `declare` against a resource infra created, and a `Retry` on those three backends republishes into the consumed resource until each gains its in-place redelivery.
`nats_external_stream()` remains for one release as a deprecated alias.
Ownership also binds how an outcome settles: a consumer never writes into an external topology, so an external topology implies `RetryStrategy::InPlace` on every backend.
Kafka and NATS implement it on this version, each with its broker's own primitive: Kafka pauses the assignment and waits, and NATS naks with a delay and counts attempts from the delivery info.
SQS (the visibility timeout), RabbitMQ (`basic.nack` with requeue) and Redis Streams (idle redelivery of a pending entry) adopt theirs when their declarers gain the verification step; [Outcomes](/concepts/outcomes) has the table.
On SQS, infra owns the topic, the queue, the queue policy, the subscription and the redrive policy, so `dlq()` on an external queue will be verify-only or refused, and shove never sets a redrive policy it does not own.
