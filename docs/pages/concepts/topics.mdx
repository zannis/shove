...
Ownership says who creates the resource, not how a `Retry` is carried out: what an external topology implies for a consumer's retry strategy is declared per backend, on that backend's page.
