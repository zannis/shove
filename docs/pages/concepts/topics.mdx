...
Two backends can bind to infrastructure that infra owns instead of creating it: `nats_external_stream()` on NATS and `kafka_external_topic()` on Kafka.
With either flag, `declare` verifies that the stream or topic exists and fails fast when it does not, while shove still creates its own DLQ.
