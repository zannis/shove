...
The rule runs both ways: `ConsumerSupervisor` and the direct `run` methods refuse an options value that sets a start, because a competing consumer starts at its committed position and never reads it.
