...
A subscriber may opt into the head of the retained log or a point in time with `ConsumerOptions::with_broadcast_start`, which takes the backend-neutral `BroadcastStart { Tail, Head, Timestamp(ms) }`.
Kafka honours all three starts on this version.
Every other backend starts at the tail only, and refuses `Head` and `Timestamp` at `subscribe()` with a `ShoveError::Topology` instead of subscribing at the tail silently.
The rule runs both ways: `ConsumerSupervisor` and the direct `run` methods refuse an options value that sets a start.
A competing consumer starts at its committed position and never reads a start.
A `ConsumerGroup` takes a group config that cannot express a start at all.
Deliver-new stays the default everywhere.
...
The tail is the default start, not the only one.
`ConsumerOptions::with_broadcast_start` takes a `BroadcastStart`, and Kafka honours all three variants.
`Tail` is the default, and `Head` replays every retained record before tailing.
`Timestamp(ms)` starts at the first record at or after that instant, resolved with the same lookup `reset_consumer_group_offsets` uses.
A partition added while the subscription runs is assigned at the same start.
See [Starting a broadcast subscription elsewhere than the tail](/backends/kafka#starting-a-broadcast-subscription-elsewhere-than-the-tail).
...
`ConsumerOptions::<Kafka>::with_group_id` replaces the string verbatim, and it stays just as inert.
That matters on a cluster whose ACLs grant group Describe on one prefix only.
librdkafka looks up the configured group's coordinator even for an assign-only handle.
The subscription tolerates the `GroupAuthorizationFailed` that lookup then draws: it warns once and keeps fetching.
An id under the granted prefix avoids provoking it.

A deferred record is redelivered to the deferring subscription before any record received after it.
The subscription's single slot is held across the delay and the same bytes are decoded again.
The order is therefore the original delivery, the redelivery, then whatever came next.
...
With `with_broadcast_start` the reconnect re-resolves the configured start instead, so a `Head` subscription replays the retained log again.
...
The start is a property of the subscription, so it is set with `with_broadcast_start` instead.
