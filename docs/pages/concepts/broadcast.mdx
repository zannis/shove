...
On Kafka a subscriber may opt into the head of the retained log or a point in time with `ConsumerOptions::<Kafka>::with_broadcast_start`.
Deliver-new stays the default there and the only behaviour elsewhere.
...
`ConsumerOptions::<Kafka>::with_broadcast_start` takes a `KafkaOffsetReset`.
`Latest` is the tail, and `Earliest` replays every retained record before tailing.
...
With `with_broadcast_start` the reconnect re-resolves the configured start instead, so an `Earliest` subscription replays the retained log again.
