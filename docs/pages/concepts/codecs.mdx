...
SNS/SQS and Redis Streams speak string payloads, so binary SBE frames fail the UTF-8 check in `encode_to_string` at publish time — the same behavior as Protobuf on those backends.
